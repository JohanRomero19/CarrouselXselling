...
def aggregate_pays(pays_norm: pd.DataFrame) -> pd.DataFrame:
    ...
    agg = (
        pays_norm.groupby(["day", "user_id", "value_prop"], as_index=False)
                 .agg(payments_total=("total", "sum"),
                      payments_cnt=("total", "size"))
    )
    return agg
...
def aggregate_events_wide(events_norm: pd.DataFrame, prefix: str) -> pd.DataFrame:
    ...
    long = (
        events_norm.groupby(["day", "user_id", "value_prop", "position"], as_index=False)
                   .size()
...
def funneling(pays_aggregated:pd.DataFrame, prints_aggregated:pd.DataFrame, taps_aggregated:pd.DataFrame, how) -> pd.DataFrame:
    """
    ...
    try:
        merged_df = (
            pays_aggregated.merge(prints_aggregated, on=["day", "user_id", "value_prop"], how=how)
                    .merge(taps_aggregated,   on=["day", "user_id", "value_prop"], how=how)
                    .fillna(0)
        )

        # convert to numeric columns
        num_cols = [c for c in merged_df.columns if c not in ["day", "user_id", "value_prop"]]
        merged_df[num_cols] = merged_df[num_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
        # sort columns
        base = ["day", "user_id", "value_prop", "payments_total", "payments_cnt",
                "prints_total", "taps_total"]
        pos_cols = [f"prints_pos_{k}" for k in range(4)] + [f"taps_pos_{k}" for k in range(4)]
        cols = [c for c in base if c in merged_df.columns] + [c for c in pos_cols if c in merged_df.columns] + \
...
def compute_metrics(funnel_df: pd.DataFrame) -> pd.DataFrame:
    """
        Add metrics to the data by (day, user_id, value_prop):
        - ctr = taps_total / prints_total
        - conv_pay_per_tap = payments_cnt / taps_total
        - conv_pay_per_user = 1 if the user pays (>0), 0 otherwise (usefull to get rates)
        (Aditional get ctr by pos, usefull to rate by pos)
    """
    ...
    df["ctr"] = safe_div(df.get("taps_total", 0), df.get("prints_total", 0))
    df["conv_pay_per_tap"] = safe_div(df.get("payments_cnt", 0), df.get("taps_total", 0))
    df["conv_pay_per_user"] = (df.get("payments_cnt", 0) > 0).astype(int)

    for k in range(4):
        p_col = f"prints_pos_{k}"
        t_col = f"taps_pos_{k}"
        df[f"ctr_pos_{k}"] = safe_div(df.get(t_col, 0), df.get(p_col, 0))
